...
-   TIFF(Tag Image File Format) is a type of file that stores geographic information in the form of raster. [@geohabnet] has been tested using *.tif* and *.nc* files but it would be able to handle any raster type of file supported by the R package *terra* [@terra] .
...
For the purposes of demonstration, the code below sets the habitat parameter programatically or directly in RStudio. Alternatively, the parameters can be fed via `parameters.yaml` in an interactive way using either the path or setting `iwindow = TRUE` for the windows prompt in `set_parameters()`
