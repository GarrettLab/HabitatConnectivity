...
This section is designed for users already familiar with R and RStudio. While RStudio is considered an interactive environment, using R from the CLI is not. The pre-print for application paper is can be accessed here [@sulá2025] .
...
Note that desc::desc() is a function from an external package and requires installation for its use. Now, the geohabnet [@geohabnet2025] package can be loaded into the current R environment.
