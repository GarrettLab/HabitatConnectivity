...
-   Package documentation - <http://geohab-site.s3-website.us-east-2.amazonaws.com/docs/>
...
Get the initial configuration file. By default this function will save the file in temporary directory `tempdir()`, however we recommend saving it to specific path e.g current working directory, `getwd()` . Using *iwindow = TRUE* will prompt a selection window to save config file.

```{r}
config_file <- get_parameters(out_path = getwd())
config_file
```
