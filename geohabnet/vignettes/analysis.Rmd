...
5.  resolution - Numeric. This is a resolution value. In the context of *SpatRaster*, it's the number of pixels that are aggregated to produce a new finer/coarser version raster data. Default is
    ...
    # setting resolutin via here will not persist the value but instead will assign it to current environment until next load_parameters() or set_parameters()
    set_reso(12)
    ```
...
```{r message=FALSE}
georast <- sean(avocado)
connectivity(georast)
```

is equivalent to `sensitivity_analysis()` or `msean()` which produces maps as side effect.
...
ss@rasters$global_rast[[1]]$east[[1]]$amatrix
```
