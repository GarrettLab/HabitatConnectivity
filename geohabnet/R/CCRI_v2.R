...
CalculateCCRI <- function(linkThreshold = 0,
  ...
  CCRI_powerlaw(config$`CCRI parameters`$DispersalParameterBeta, linkThreshold,
  ...
  CCRI_negative_exponential(config$`CCRI parameters`$DispersalParameterGamma, linkThreshold,
...
SensitivityAnalysisOnGeoExtentScale <- function(linkThreshold = 0, geoScale, aggregateMethods, cropHarvestRaster, hostDensityThreshold, resolution) {
  ...
  terra::plot(mean_index_raster,
    ...
    main = paste(
      "Mean cropland connectivity risk index from sensitivity analysis: ",
      config$`CCRI parameters`$Crops, "resolution = ", config$`CCRI parameters`$Resolution
    ), cex.main = 0.7
  )
  raster::plot(rworldmap::countriesLow, add = TRUE)
...
SenstivityAnalysis <- function() {
  ...
  # crop data
  cropharvest <- get_cropharvest_raster_sum(as.list(config$`CCRI parameters`$Hosts)) # list
  ...
  lapply(geoScales, SensitivityAnalysisOnCroplandThreshold,
    linkThresholds = config$`CCRI parameters`$LinkThreshold, hostDensityThresholds = croplandThresholds, aggregateMethods = aggregateMethods,
