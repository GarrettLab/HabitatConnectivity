...
test_that("calculate_metrics_weight returns correct weights", {
  ...
  expect_equal(
    unname(calculate_metrics_weight(node_strength = TRUE, sum_of_nearest_neighbors = TRUE, eigenvector_centrality = TRUE)),
    c(0, 3, 3, 3)
  )
  
  expect_equal(
    unname(calculate_metrics_weight(betweenness_metric = TRUE, sum_of_nearest_neighbors = TRUE, eigenvector_centrality = TRUE)),
    c(2, 0, 4, 4)
  )
  
  expect_equal(
    unname(calculate_metrics_weight(betweenness_metric = TRUE, node_strength = TRUE, sum_of_nearest_neighbors = TRUE, eigenvector_centrality = TRUE)),
    c(2, 6, 6, 6)
  )
})
